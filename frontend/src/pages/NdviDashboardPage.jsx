...
export default function NdviDashboardPage() {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const blob = await reportApi.downloadTerrainReport(terrainId)
      const url  = URL.createObjectURL(blob)
      const a    = document.createElement('a')
      a.href     = url
      a.download = `SIMGAN-Informe-${dashboard?.terrainName || terrainId}.pdf`
  ...
  return (
    <div className="page-container">
      ...
      <div className="page-header">
        ...
        <TerrainNav name={dashboard?.terrainName} />
        <div className="flex justify-between items-center">
          ...
          <div className="ndvi-analysis-controls">
            ...
            <button
              ...
              disabled={downloadingPdf || !hasData}
              title={!hasData ? 'Ejecuta un análisis primero para generar el informe' : 'Descargar informe PDF completo'}
              style={{ display: 'flex', alignItems: 'center', gap: 6 }}
            >
              {downloadingPdf ? <><span className="spinner" /> Generando...</> : '📄 Descargar Informe PDF'}
            </button>
          </div>
        </div>
      ...
      {!hasData ? (
        ...
        <>
          ...
          <div className="flex gap-12 mt-24">
            ...
            >
              {downloadingPdf ? <><span className="spinner" /> Generando PDF...</> : '📄 Descargar Informe PDF'}
            </button>
          </div>
        </>
      )}
